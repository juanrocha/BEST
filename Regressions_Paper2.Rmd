...
```{r fig1, fig.height= 2, fig.width= 6, include = TRUE, echo = FALSE, message=FALSE, warning=FALSE, fig.align='center', fig.cap = 'Comparison of linear models. (A) individual extraction and (B) proportion of stock available as response variable. SE estimation using HC1 (Stata default) and are clustered around groups.', fig.pos = "h", out.extra= ''}
...
stargazer::stargazer(
  ...
  type = "latex", header = FALSE, summary = FALSE, #float = FALSE,
...
```{r fig3, include = TRUE, echo = FALSE, fig.height= 2, fig.width=5, fig.cap = "Difference-in-difference comparison for control and treated groups for (A) individual extraction as number of fish, (B) as proportion of available stock size, and (C) cooperation.", fig.pos = "H", out.extra= '', fig.align='center'}
...
g1 <- diff_df[1:4,] %>% select(1:3) %>%
  ...
  geom_line(aes(group = treatment, color = treatment)) + 
  labs(tag = "A") +
  theme_light(base_size = 8) + theme( legend.position = "right")
...
```{r alternative_for_paper, include = TRUE, echo = FALSE}
...
diff_df4 <- bind_rows(
  ...
  mutate(response = as_factor(response, order = c(2,3,1)))
...
# ggsave(g_diff, filename = "diff-in-diff.pdf", device = "pdf", width = 3, height = 3, units = "in", dpi = 600 )
...
pm <- GGally::ggpairs(
  ...
  columns = c(3:6,14,52),
  aes(color = Treatment, fill = Treatment, alpha = 0.5),
...
# ggsave(g_reg2, filename = "regression_cooperation.png", device = "png", width = 6, height = 4, units = "in", dpi = 300 )
g_reg2
```
...
```{r fig2, include = TRUE, echo = FALSE, fig.height=7, fig.width=7, fig.align='center', warning=FALSE, message=FALSE, dev.args= list(pointsize = 8), fig.cap = "What drives the behavior of individuals."}
...
g2 <- ind_coop %>% ungroup() %>%
  ...
  geom_point(aes(color = Treatment), size = 0.5, 
               alpha = 0.7, 
               show.legend = FALSE) + 
  geom_text(
    data = data.frame(
...
g5 <- ind_coop %>% ungroup() %>%
  filter(part == TRUE) %>%
  select(Treatment, mean_extraction, mean_prop_extr, var_extraction, var_prop_extr) %>%
...
# quartz.save(file="fig2_model.pdf", type = "pdf", width = 7, height = 7, pointsize = 8, dpi = 600)
...
```{r}
# save.image(file = "Regressions_paper2.RData", safe = TRUE)
```

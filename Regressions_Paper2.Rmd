...
ind_coop <- dat %>%
  ...
  summarize(
    mean_extraction = median(ind_extraction, na.rm = TRUE),
    mean_prop_extr = median(stock_ratio, na.rm = TRUE),
...
```{r fig3, include = TRUE, echo = FALSE, fig.height= 2, fig.width=5, fig.cap = "Difference-in-difference comparison for control and treated groups for (A) individual extraction as number of fish, and (B) as proportion of available stock size.", fig.pos = "H", out.extra= '', fig.align='center'}
...
# ggsave(g_diff, filename = "diff-in-diff.png", device = "png", width = 5, height = 5, units = "in", dpi = 600 )
...
pm <- GGally::ggpairs(
  ...
  upper = list(continuous = wrap("cor", size = 3, alignPercent = 1, hjust = 0))
  )

pm + theme_light(base_size = 10) + theme(legend.position = "bottom")
```
